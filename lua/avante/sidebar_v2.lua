...
---@field containers { result?: NuiSplit, todos?: NuiSplit, selected_code?: NuiSplit, input?: NuiSplit }
...
function Sidebar:new(id)
  return setmetatable({
    ...
    old_result_lines = {},
    token_count = nil,
    use_inline_file_references = true,
...
function Sidebar:on_mount(opts)
  ...
  if Config.behaviour.auto_set_keymaps and Config.mappings.files and Config.mappings.files.add_current then
    vim.keymap.set("n", Config.mappings.files.add_current, function()
      if self:is_open() then
        self:add_current_buffer_reference()
        vim.notify("Added current buffer reference to input", vim.log.levels.DEBUG, { title = "Avante" })
      else
        vim.notify("Failed to add current buffer reference", vim.log.levels.WARN, { title = "Avante" })
      end
    end, {
      desc = "avante: add current buffer reference",
      noremap = true,
      silent = true,
    })
  end
...
function Sidebar:initialize()
  ...
  -- Keep file selector for API/tool compatibility, but v2 uses inline links as source of truth.
...
---@return integer | nil
function Sidebar:get_input_bufnr()
...
---@param paths string[]
function Sidebar:append_file_reference_links(paths)
  local bufnr = self:get_input_bufnr()
  if not bufnr then return end
  ReferenceLinks.append_paths_to_buffer(bufnr, paths)
end

---@param paths string[]
function Sidebar:remove_file_reference_links(paths)
  local bufnr = self:get_input_bufnr()
  if not bufnr then return end
...
---@param filepath string
function Sidebar:add_selected_file_reference(filepath)
  self:append_file_reference_links({ filepath })
end

---@param filepath string
function Sidebar:remove_selected_file_reference(filepath)
  self:remove_file_reference_links({ filepath })
end

function Sidebar:add_current_buffer_reference()
  local current_buf = vim.api.nvim_get_current_buf()
  local filepath = vim.api.nvim_buf_get_name(current_buf)
  if filepath and filepath ~= "" then self:add_selected_file_reference(filepath) end
end

function Sidebar:add_buffer_file_references()
  local paths = {}
  ...
  self:append_file_reference_links(paths)
end

function Sidebar:add_quickfix_file_references()
  local paths = vim
    .iter(vim.fn.getqflist({ items = 0 }).items)
    :filter(function(item) return item.bufnr ~= 0 end)
    :map(function(item) return vim.api.nvim_buf_get_name(item.bufnr) end)
...
function Sidebar:get_generate_prompts_options(request, opts, cb)
  ...
  table.insert(tools, {
    ...
    func = function(input)
      self:add_selected_file_reference(input.rel_path)
  ...
  table.insert(tools, {
    ...
    ---@type AvanteLLMToolFunc<{ rel_path: string }>
    func = function(input)
      self:remove_selected_file_reference(input.rel_path)
  ...
  local selected_filepaths = opts.selected_filepaths or self.file_selector.selected_filepaths or {}
...
function Sidebar:handle_submit(request)
  ...
  local selected_filepaths = ReferenceLinks.extract_paths(request)
  ...
  api.nvim_create_autocmd("WinClosed", {
    ...
    callback = function(args)
      ...
      if closed_winid then
        ...
        -- Ignore closing todos window because it can disappear during normal operation
        if container and container ~= self.containers.todos then
          self:close()
        end
      end
    end,
...
function Sidebar:adjust_selected_files_container_layout() end
...
function Sidebar:create_selected_files_container() end
