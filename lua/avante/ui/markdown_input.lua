...
---@field enable_markdown boolean
local MarkdownInput = {}
MarkdownInput.__index = MarkdownInput

---@class avante.ui.MarkdownInputOptions
---@field start_insert? boolean
...
---@param opts? avante.ui.MarkdownInputOptions
function MarkdownInput:new(opts)
  opts = opts or {}
  local obj = setmetatable({}, MarkdownInput)
  obj.bufnr = nil
  obj.winid = nil
  obj.shortcuts_hints_winid = nil
  obj.augroup = api.nvim_create_augroup("MarkdownInput", { clear = true })
  ...
  obj.enable_markdown = opts.enable_markdown ~= false -- Default to true for this class
  obj.spinner_chars = Config.windows.spinner.editing
  obj.spinner_index = 1
  obj.spinner_timer = nil
  obj.spinner_active = false
  obj.popup_hint_id = vim.api.nvim_create_namespace("avante_markdown_input_hint")
  return obj
end

---Setup markdown treesitter and render-markdown integration
function MarkdownInput:setup_markdown()
  if not self.bufnr or not api.nvim_buf_is_valid(self.bufnr) then return end
  if not self.enable_markdown then return end
  ...
  vim.b[self.bufnr].avante_markdown_enabled = true
  vim.b[self.bufnr].render_markdown_config = {
    enabled = true,
...
function MarkdownInput:close_shortcuts_hints()
...
function MarkdownInput:start_spinner()
...
function MarkdownInput:stop_spinner()
...
function MarkdownInput:setup_keymaps()
...
function MarkdownInput:setup_autocmds()
  local bufnr = self.bufnr
