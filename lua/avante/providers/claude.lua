...
---@field auth_type "api" | "max" -- "max" is deprecated and blocked; use ACP with Claude Code instead.

---@class AvanteProviderFunctor
local M = {}

local max_auth_notification =
  "ACP with Claude Code should be used instead, as using Claude subscriptions outside of Anthropic products is disallowed"
...
M._is_setup = false
M._max_auth_notification_shown = false

local function notify_max_auth_type()
  if M._max_auth_notification_shown then return end
  M._max_auth_notification_shown = true
  Utils.warn(max_auth_notification, { once = true, title = "Avante Claude" })
end
...
function M.setup()
  ...
  if auth_type == "max" then
    notify_max_auth_type()
    return
  end
end
