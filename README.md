...
```lua
{
  ...
  providers = {
    claude = {
      endpoint = "https://api.anthropic.com",
      auth_type = "api", -- "max" is unsupported; use ACP with Claude Code instead
...
Claude subscription auth through the traditional `claude` provider is unsupported. Use ACP with Claude Code instead.

See the `Claude Code with ACP` section below for the supported configuration path.
